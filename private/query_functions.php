<?php
  ...
  function validate_user_update($user, $password, $confirm_password, $errors=array()) {
    if (is_blank($user['first_name'])) {
      $errors[] = "First name cannot be blank.";
    } elseif (!has_length($user['first_name'], array('min' => 2, 'max' => 255))) {
      $errors[] = "First name must be between 2 and 255 characters.";
      ...
      $errors[] = "Username not allowed. Try another.";
		...
		if(!is_blank($password) || !is_blank($confirm_password)) { 
			if(is_blank($password)) {
				$errors[] = "password cannot be blank.";
			} elseif (is_blank($confirm_password)) {
				$errors[] = "confirm password cannot be blank.";
			} elseif ($password != $confirm_password) {
				$errors[] = "passwords do not match.";
			} elseif (!has_length($password, array('min' => 12, 'max' => 255))) {
				$errors[] = "password must be at least 12 characters long.";
  ...
  function insert_user($user) {
    ...
    $sql .= "'" . db_escape($db, $user['username']) . "',";
		$sql .= "'" . db_escape($db, password_hash($user['password'], PASSWORD_DEFAULT)) . "', ";
    $sql .= "'" . $created_at . "'";
    $sql .= ");";
    // For INSERT statements, $result is just true/false
    $result = db_query($db, $sql);
  ...
  // Edit a user record
  // Either returns true or an array of errors
  function update_user($user, $password, $confirm_password) {
    global $db;

    $errors = validate_user_update($user, $password, $confirm_password);
    if (!empty($errors)) {
      return $errors;
    }
    	...
    	$sql .= "username='" . db_escape($db, $user['username']) . "', ";
			$sql .= "hashed_password='" . db_escape($db, password_hash($password, PASSWORD_DEFAULT)) . "' ";
